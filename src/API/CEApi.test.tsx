...
describe("CE Api Annotation parsing", () => {
    it("target is url", () => {
        const target_url = require('./testdata/target-url.json')
        let ann = TrompaClient.annotationFromCE(target_url, "http://localhost:4000/")
        expect(ann.target.length).toEqual(1);
        expect(ann.target[0]).toBeInstanceOf(AnnotationTarget);
        expect(ann.target[0].id).toEqual("https://example.com/audio.mp3")
    })
    it("target is node without field", () => {
        const target_node = require('./testdata/target-node-only.json')
        let ann = TrompaClient.annotationFromCE(target_node, "http://localhost:4000/")
        const expected = {
            "end": undefined,
            "fieldName": null,
    ...
    it("target is node with contentUrl field (MediaObject)", () => {
        ...
        let ann = TrompaClient.annotationFromCE(target_contenturl_fragment, "http://localhost:4000/")
        let expected = {
            "end": 20,
    ...
    it("has motivation", () => {
        ...
        let ann = TrompaClient.annotationFromCE(motivation, "http://localhost:4000/")
    ...
    it("has url body", () => {
        ...
        let ann = TrompaClient.annotationFromCE(body, "http://localhost:4000/")
        // These will be parsed to an AnnotationExternalWebResource
        let expected = [
            "https://trompamusic.eu/something",
            "http://example.com/somethingelse"
        ]
        expect(ann.body?.[0].id).toEqual(expected[0])
        expect(ann.body?.[1].id).toEqual(expected[1])
    })
    it("has textual body", () => {
        ...
        expect(ann.body?.length).toEqual(1)
        expect(ann.body?.[0]).toEqual(expected)
    })
